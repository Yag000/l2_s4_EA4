...
def puissance_matrice_2_2(M, n):
    ...
    return produit_matrice_2_2(M, carre)
...
# On observe que fibo_4 est dans al même classe que x^log_2(3) (car on utilise
# l’exponentiation de matrice) ce qui est plus efficace que x²(fibo_3)
...
def puissance_matrice_2_2_ops(M, n):
    ...
    tmp, ops = puissance_matrice_2_2_ops(M, n // 2)
    carre, ops2 = produit_matrice_2_2_ops(tmp, tmp)
    ops += ops2

    if n % 2 == 0:
        return carre, ops

    value, op3 = produit_matrice_2_2_ops(M, carre)
    return value, ops + op3
...
# On observe a nouveau la linéarité du nombre d'additions de fibo_3 et aussi le comportement logarithmique que fibo_4.
...
def decompte_naif(a, b, c, d, e):
    ...
    return nbOfBits(b * c) ** 2 + nbOfBits(d * e) ** 2
...
def decompte_karatsuba(a, b, c, d, e):
    ...
    return nbOfBits(b * c) ** log2(3) + nbOfBits(d * e) ** log2(3)
...
def produit_matrice_2_2_bits(M1, M2, compte=decompte_naif):
    ...
    res[0][0] = M1[0][0] * M2[0][0] + M1[0][1] * M2[1][0]
    ops += compte(res[0][0], M1[0][0], M2[0][0], M1[0][1], M2[1][0])

    res[1][0] = M1[1][0] * M2[0][0] + M1[1][1] * M2[1][0]
    ops += compte(res[1][0], M1[1][0], M2[0][0], M1[1][1], M2[1][0])

    res[0][1] = M1[0][0] * M2[0][1] + M1[0][1] * M2[1][1]
    ops += compte(res[0][1], M1[0][0], M2[0][1], M1[0][1], M2[1][1])

    res[1][1] = M1[1][0] * M2[0][1] + M1[1][1] * M2[1][1]
    ops += compte(res[1][1], M1[1][0], M2[0][1], M1[1][1], M2[1][1])
...
def puissance_matrice_2_2_bits(M, n, compte=decompte_naif):
    ...
    tmp, ops = puissance_matrice_2_2_bits(M, n // 2, compte)
    carre, ops2 = produit_matrice_2_2_bits(tmp, tmp, compte)
    ops += ops2

    if n % 2 == 0:
        return carre, ops

    value, op3 = produit_matrice_2_2_bits(M, carre, compte)
    return value, ops + op3
...
#  On observe que fibo_4 n'est pas plus efficace en termes de bits que fibo_3 si on
# utilise un algorithme de multiplication naif, qui est quadratique en le nombre de bits
# des facteurs. Par contre l'utilisation de karatsuba permet d'obtenir un algorithme
# plus efficace en termes de bits, ergo en temps d'exécution. Ceci se doit au fait que
# l'algorithme de karatsuba permet de réduire le nombre de multiplications effectuées.
...
def test_puissanceData():
    return [
        ...
        [
            [[3, 12], [17, 26]],
            8,
            [[252018687525, 442324154508], [626625885553, 1099806650332]],
        ],
        [[[20, 16], [5, 10]], 6, [[192672000, 202176000], [63180000, 66312000]]],
        [
            [[232, 188], [70, 65]],
            5,
            [[1640021237352, 1367968885308], [509350116870, 424857387105]],
        ],
        [[[10, 7], [2, 3]], 4, [[15362, 12467], [3562, 2895]]],
        [
            [[60, 5], [4, 1]],
            7,
            [[2894868940020, 243934318805], [195147455044, 16443978121]],
        ],
        [
            [[54, 30], [11, 4]],
            10,
            [
                [538139813380894176, 288789026527531200],
                [105889309726761440, 56824769168342176],
            ],
        ],
        ...
        [
            [[60, 5], [4, 1]],
            9,
            [[10538945536660820, 888057647401005], [710446117920804, 59865297328961]],
        ],
    ]
...
def test_fibo_4():
    ...
    for i, dt in enumerate(data):
        ...
        if fb == Tres and ops in Tops:
            ...
            print(
                "    attendu : %d en %s ops"
                % (Tres, (" ou ".join([str(i) for i in Tops])))
            )
        else:
            printcol("{Mauvais résultat}", "red")
            print("    entrée  : %s" % n)
...
if __name__ == "__main__":
    ...
    courbes_bits(10000, pas=10)  # fibo_3 et fibo_4, multiplication naïve
    courbes_bits(
        10000, pas=10, compte=decompte_karatsuba
    )  # fibo_3 et fibo_4, multiplication par Karatsuba
    courbes_bits(
        10000, tous=False, pas=10, compte=decompte_karatsuba
    )  # seulement fibo_4, multiplication par Karatsuba
